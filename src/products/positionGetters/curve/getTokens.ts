...
export const getTokens = async (provider: any, activeNetwork: NetworkConfig, metadata?: any): Promise<Token[]> => {
  ...
  const touchedCurveLpAddresses = metadata.transferHistory
    .filter((r: any) => r.tokenName.startsWith('Curve.fi'))
  ...
  const tokens: Token[] = []
  for (let i = 0; i < uniqueCurveLpAddresses.length; i++) {
    ...
    const lpTokenContract = getContract(uniqueCurveLpAddresses[i], ierc20Json.abi, provider)
    ...
    console.log(uniqueCurveLpAddresses[i], balance)
    ...
    if (balance.gt(ZERO)) {
      ...
      const token = {
        token: {
          address: uniqueCurveLpAddresses[i],
          name: poolName,
          symbol: symbol,
          decimals: decimals,
          balance: ZERO,
