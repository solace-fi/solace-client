...
import { withBackoffRetries, decimals, range } from '../../../'
...
export const getBalances = async (user: string, provider: any) => {
  ...
  let indices = range(tokens.length)
  ...
  indices.forEach(
    (i) =>
      (balances[i].underlying.balance = formatEther(
        parseEther(balances[i].token.balance).mul(exchangeRates[i]).div(String(POW_EIGHTEEN))
      ))
  )
